...
class BitField(object):
    ...
    def extract(self, word: int) -> int:
        ...
        """
        # You will need shift the word right with the >> operation, mask it, and return the result.
    ...
    def insert(self, value: int, word: int) -> int:
        ...
        """
        trim = (self.mask & value) << self.from_bit
        result = trim | word
        return result
    ...
    def extract_signed(self, word: int) -> int:
        ...
        # First extract the field as an unsigned integer, and then sign-extend it. I leave it to you.
        field = self.extract(word)
        width = (self.to_bit - self.from_bit) + 1
        # sign extend = shift bits right >> w/o changing values
        return sign_extend(field, width)

...
# fields will be treated as unsigned integers (non-negative values only)
...
def decode(word: int) -> "Instruction":
    ...
    my_op = OpCode(instr_field.extract(word))
    my_cond = CondFlag(cond_field.extract(word))
    my_reg_target = reg_target_field.extract(word)
    my_reg_src1 = reg_src1_field.extract(word)
    my_reg_src2 = reg_src2_field.extract(word)
    my_offset = offset_field.extract_signed(word)
    return Instruction(my_op, my_cond, my_reg_target, my_reg_src1, my_reg_src2, my_offset)
